...
class Actor:
    ...
    def change_icon(self, icon_file):
        '''(Actor, str) -> None
        Given an icon_file name then change the icon.
        '''
    ...
    def step(self):
        ...
        if self._last_event is not None:
            ...
            if self._last_event == pygame.K_UP:
                dx, dy = 0,-1
            # Assign keys for diagonal movement.
...
class Box(Actor):
    ...
    def move(self, other, dx, dy):
        ...
        check_coor = current_stage.get_actor(new_x, new_y)
        # Check if the new position is on stage and the spot is avilable.
        if on_stage and check_coor == None:
            # Set the res to True
            res = True
            Actor.move(self, other, dx, dy)
        # If the space is occupied, Ask the Actor to move.
            ...
            check_coor = current_stage.get_actor(new_x, new_y)
            if(check_coor == None):
                # Move the Actor to the new spot
                Actor.move(self, other, dx, dy)
                # Set res to True
                res = True
            else:
...
class Stage:
    ...
    def get_pixel_width(self):
        '''(Stage) -> int
        Return the width of stage in pixel
        '''
        return self._icon_dimension * self._width
    
    def get_picel_height(self):
        '''(stage) -> int
        Return he height of stage in pixel
        '''
...
class invisibleMonster(Monster):
    ...
    If the player touch them or the monster touches them, then you start at (0,0)
    If (0,0) is not empty then player get trasported to a random (x,y)
    '''
