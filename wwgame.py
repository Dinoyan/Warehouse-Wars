...
text = font2.render("Warehouse Wars", True, (0, 128, 0))
...
while not done and display_instructions:
    ...
    if start_screen == 1:
        ...
        width = ww.get_width()
        height = ww.get_height()
        ww._screen.fill((255, 255, 255))
        ww._screen.blit(text,[width/2, height/2])
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if any(isinstance(actor, Monster) for actor in actors_lst):
            print('yes')       
        else:
            print('no')
